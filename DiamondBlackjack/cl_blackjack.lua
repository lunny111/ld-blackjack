...
AddEventHandler("tp-casino:casinoExitedEvent", function()
  inCasino = false
...
Citizen.CreateThread(function()
  --Wait(10000)
  dealerPedSeeds = RPC.execute("tp-blackjack:getDealerPedSeeds")
  -- TriggerEvent("tp-casino:casinoEnteredEvent")
end)
                ...
                if not timeoutHowToBlackjack then
                    if not uiIntShown then
                        ...
                        local background = "info"
                            ...
                            exports["ld-ui"]:DrawNotify("blackjack", "[E] Play! " ..dwGetLimitTitleString(), "#147efb", "fas fa icons")
                        uiIntShown = true
                    end
                end
...
Citizen.CreateThread(function()
    while true do
        if drawCurrentHand then -- @HERE
          --prevCurrentHand = currentHand
          if dealersHand ~= 0 and currentHand ~= 0 then
            text1 = "Dealer: " .. tostring(dealersHand)
            text2 = "You: " .. tostring(currentHand)
          else
            text1 = "Waiting for dealer..." 
            text2 = ""
           end
           TriggerServerEvent("Blackjack:svDrawText", drawCurrentHand, text1, text2)
        end
        CasinoWaitTimer()
    end
end)

RegisterNetEvent("Blackjack:clDrawText")
AddEventHandler("Blackjack:clDrawText",function(display,draw1,draw2)
    if display then
        ESX.TriggerServerCallback("Blackjack:getCasinoChips", function(balance)
        exports["tp-textdisplay"]:DisplayText({
            header = "Blackjack " .. dwGetLimitTitleString(),
                rows = {
                {
                    id = 0, 
                    txt = "Balance: $" .. format_int(balance)
                },
                {
                    id = 1, 
                    txt = draw1
        ...
        exports["tp-textdisplay"]:HideText()
    end
...
RegisterUICallback("ld-ui:blackjackHitStand", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["ld-ui"]:hideContextMenu(data)
  ...
  elseif action == "double" then
    local dwUseChipsResult = RPC.execute("tp-casino:useChips", currentBetAmount)
  ...
  elseif action == "surrender" then
    ...
    -- RPC.execute("tp-casino:winChips", currentBetAmount / 2)
...
AddEventHandler("ld-ui:application-closed", function(name)
...
function dwIdleState(act)
  if act and not isDwIdleStateActive then
    ...
    text1 = "Waiting for dealer..."
    text2 = ""
    TriggerServerEvent("Blackjack:svDrawText", true, text1, text2)
        ...
        if waitingForStandOrHitState and sittingAtBlackjackTable and blackjackGameInProgress then
            if not shownHitStandContextOptions then
              ...
              if dwTotalCardsDealt == 2 then
                data[#data + 1] = {
                  action = "ld-ui:blackjackHitStand",
...
RegisterUICallback("ld-ui:blackJackBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["ld-ui"]:hideContextMenu(data)
  -- exports["ld-ui"]:closeApplication("textbox")
  currentBetAmount = tonumber(data.key)
  if not currentBetAmount < balance then 
  ...
  if currentBetAmount and currentBetAmount >= limits.min and currentBetAmount <= limits.max then
      ...
      if dwUseChipsResult then
        ...
        putBetOnTable()
        Wait(1000)
        text1 = "Bet: $" .. format_int(currentBetAmount)
        text2 = ""
        TriggerServerEvent("Blackjack:svDrawText", true, text1, text2)
...
AddEventHandler("ld-ui:application-closed", function(name, data)
...
function dwWaitForBet(act)
  ...
  if waitingForBetState then
    ...
    if sittingAtBlackjackTable then
      ...
      local data = {}
      data[#data + 1] = {
        action = "ld-ui:blackJackBet",
        title = "Place bet",
        description = dwGetLimitTitleString(),
        key = limits.max,
      }
      exports["ld-ui"]:showContextMenu(data)
      text1 = "Waiting for bet..."
      text2 = ""
      TriggerServerEvent("Blackjack:svDrawText", true, text1, text2)
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    -- RPC.execute("tp-casino:winChips", currentBetAmount)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
...
AddEventHandler("Blackjack:blackjackWin",function(tableId, dwHitBlackjack)
    ...
    -- RPC.execute("tp-casino:winChips", (currentBetAmount * 2) + extra)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
