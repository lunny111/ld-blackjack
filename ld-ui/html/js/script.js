...
function correct(){
    ...
    $.post('https://ld-ui/ld-ui:taskBarSkillResult', JSON.stringify({
        success: true,
    }));
}
...
const CloseMenu = () => {
    ...
    saved = "";
    textboxactive = false
    $.post(`https://ld-ui/guccimanecancel`)
...
const PostData = (data) => {
    return $.post(`https://ld-ui/`+textmenucallbackurl, JSON.stringify(data))
}

function closesettings(){
    $(".settings").hide();
    $.post('https://ld-ui/close');
}
    ...
    switch (action) {
        ...
        case "eye":
            ...
            if (msg.data.action == "refresh") {
                ...
                $.post('https://ld-ui/closecontext', JSON.stringify({}));
            }
            break;
        case "sniper-scope":
...
$(document).keydown(function(event) {
    ...
    if (key == 27) {
        ...
        if (eyeintactive){
            eyeintactive = false
            $.post('https://ld-ui/closeeyeint', JSON.stringify({}));
        }
    }
});
        ...
        if (event.target.id == "backmenu") {
            ...
            if (event.target.id.split("-")[0] === 'contextmenu') {
                ...
                if (children) {
                    ...
                    $(".buttoncontext").html("");
                    $(".buttoncontext").append('<div class="button-context" id="backmenu"><div class="titlecontext" id="backmenu">< Voltar</div><div class="descriptioncontext" id="backmenu"></div></div>');
                    ...
                    $(".buttoncontext").html("");
                    $(".containercontext").hide();
                    $.post('https://ld-ui/'+action, JSON.stringify({
                        key:key
                    }));
                ...
                $(".buttoncontext").html("");
                $(".containercontext").hide();
                $.post('https://ld-ui/'+subaction, JSON.stringify({
                    key:subkey
                }));
            }
        }
